...
export default function PanelProducts() {
    ...
    async function handleDealCreate(index) {
        try {
            const dealPrice = parseFloat(prompt(`Digite o preço do(a) ${products[index].measurement_unit} com desconto`).replace(',', '*').replace('.', ',').replace('*', '.'));
            const dealUnitPrice = products[index].unit_price ? parseFloat(prompt('Digite o valor da UN com desconto').replace(',', '*').replace('.', ',').replace('*', '.')) : null;
